...
export function CodeBlock({ code, filename }: CodeBlockProps) {
  ...
  return (
    <div className="rounded-xl border border-canvas-border overflow-hidden bg-[#fafaf8] text-sm">
      {filename && (
        <div className="flex items-center justify-between px-4 py-2.5 border-b border-canvas-border bg-canvas-code">
          <span className="font-mono text-[11px] text-canvas-muted">{filename}</span>
          <button
            onClick={copy}
            className="flex items-center gap-1.5 font-mono text-[10px] text-canvas-muted hover:text-canvas-tx transition-colors"
          >
            {copied ? <><span className="text-emerald-600">✓</span> Copied</> : <><CopyIcon /> Copy</>}
          </button>
        </div>
      )}
      {!filename && (
...
function highlight(code: string): string {
  return code
    ...
    // keywords
    .replace(/\b(import|export|from|const|let|var|function|async|await|return|if|else|type|interface|default|extends)\b/g, '<span style="color:#7c3aed">$1</span>')
